...
Request(RegisterUserRequest) → Controller → RegisterUserCommand → RegisterUserUseCase
→ SaveUserPort.save(User) → UserPersistenceAdapter/JPA → 결과(RegisterUserResult)
...
[GET /api/auth/me]
AuthenticationPrincipal(publicId) → GetUserProfileQuery.getProfile(publicId)
→ LoadUserPort.loadById(UserId.from(publicId)) → UserProfileResult.from(User) → Response
```
...
Q1. 왜 DTO를 두 벌(Application, Presentation)로 나누나요?
- A. 레이어 독립성과 재사용성을 위해서입니다. 애플리케이션 결과는 웹이 아닌 다른 인터페이스(gRPC/CLI)에서도 재사용 가능해야 합니다.

Q2. publicId를 나중에 암호화/압축 형태로 바꾸고 싶으면?
- A. 포맷 변경이 필요하면 `UserId` 생성 로직과 저장/복원 매퍼만 수정하면 됩니다. 외부 계약은 여전히 `publicId` 문자열로 유지됩니다.

Q3. VO가 너무 많아지는 것 아닌가요?
- A. 규칙은 “의미/검증/행위가 있는 값”만 VO로 승격합니다. 단순 포워딩만 하는 VO는 과도한 추상화가 될 수 있으니 주의합니다.

Q4. 트랜잭션은 어디에 두나요?
- A. 애플리케이션 서비스(유스케이스) 경계에 둡니다. 도메인은 트랜잭션을 모릅니다.